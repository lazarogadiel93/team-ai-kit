...
function Save-TeamRulesToEngram {
    <#
    .SYNOPSIS
        Saves team knowledge repo rules to engram as persistent memory.
        Each rules/*.md file is saved as a separate observation associated with the project.
    .OUTPUTS
        Int — number of rules saved to engram.
    #>
    param(
        [Parameter(Mandatory)]
        [string]$RepoUrl,
    ...
    $engramBin = Get-EngramBinaryPath
    if (-not $engramBin) { return 0 }

    $localPath = Get-TeamRepoLocalPath -RepoUrl $RepoUrl
    $rulesDir = Join-Path $localPath 'rules'
    if (-not (Test-Path $rulesDir)) { return 0 }

    $rulesFiles = @(Get-ChildItem -Path $rulesDir -Filter '*.md' -Recurse | Sort-Object Name)
    if ($rulesFiles.Count -eq 0) { return 0 }

    $saved = 0
    foreach ($file in $rulesFiles) {
        $title = "Team rules: $($file.BaseName)"
        $content = Get-Content -Path $file.FullName -Raw
        if (-not $content) { continue }

        try {
            & $engramBin save $title $content --project $ProjectName --type decision 2>$null
            if ($LASTEXITCODE -eq 0) { $saved++ }
        }
        catch {
            # Engram save is best-effort -- don't fail init/update
        }
    }
...
function New-CopilotInstructions {
    <#
    .SYNOPSIS
        Generates the copilot-instructions.md content from the instructions template.
        Resolves IDE-specific skill paths. Team rules are stored in engram, not inline.
    #>
    param(
        [Parameter(Mandatory)]
        [string]$Ide,
    ...
    $templatePath = if ($KitRoot) {
        Join-Path (Join-Path $KitRoot 'templates') 'instructions.md'
    }
    else {
        Join-Path (Join-Path $PSScriptRoot '..') (Join-Path 'templates' 'instructions.md')
    }
    ...
    # Resolve skill path placeholders
    $projectSkills = if ($ProjectSkillsDir) { $ProjectSkillsDir } else { '.github/skills/' }
    $globalSkills = if ($GlobalSkillsDir) { $GlobalSkillsDir } else { '~/.copilot/skills/' }

    $result = $template.Replace('{{PROJECT_SKILLS_DIR}}', $projectSkills).Replace('{{GLOBAL_SKILLS_DIR}}', $globalSkills)
